/**
 * Excel 预览解析：xlsx 走 ExcelJS（含嵌入图），失败则回退 parseXlsFallbackPreview。
 * 供站内预览与「新标签页打开」共用。
 */
import {
...
export type ExcelPreviewParseResult =
...
async function parseWithExcelJS(
  ...
  wb.eachSheet((ws) => {
    ...
    const rows: ExcelPreviewSheetGrid['rows'] = [];
    for (let r = 0; r < rowCount; r++) {
      const row: ExcelPreviewSheetGrid['rows'][number] = [];
...
/** 解析工作簿；registerUrl 收集 blob: 以便调用方 revoke */
export async function parseExcelPreviewFromBuffer(
  buffer: ArrayBuffer,
  registerUrl: (url: string) => void,
): Promise<ExcelPreviewParseResult> {
  try {
    return { mode: 'grid', sheets: await parseWithExcelJS(buffer, registerUrl) };
  } catch {
    return parseXlsFallbackPreview(buffer, registerUrl);
  }
}
