...
- 产品字段取值：`sku`（产品名称）、`partner`（合作单位名称，仅分类开启 `linkPartner` 时可选）或 `custom:<ReportFieldDefinition.id>`（分类扩展字段，仅 text/select/date 参与）；select 可配「选项 → 编号」映射，date 按 `yyMMdd|yyMM|yy|yyyyMMdd` 格式化。
- `partner` 的值不在 `Product` 上，由调用方解析后经 `ProductCodeValueContext.partnerName` 传入拼接函数（Web `hooks/useProductCodeAutoFill.ts` 用 `resolveProductPartnerName`，小程序由 `productCodeAutoFill.setMasterData({ categories, partners })` 注入）；候选选项同理经 `ProductCodeFieldOptionContext.partnerNames` 传给配置弹窗。
- API：`GET /api/products/code-rules`（只读返回网页配置的 `productCodeRules`，权限 `requireTenantMemberRead`，供小程序新建页消费——整包 `GET /settings/config` 会放开全部租户配置，故不为取号放宽其权限）；`GET /api/products/next-code?prefix=&serialLength=`（预取号，权限 `basic:products:create` **或** `development:styles:create`；号池含产品档案与开发款式品名）；`POST /api/products` 可带 `codeAutoGen: { prefix, serialLength }`，服务端在 `product_code:{prefix}` advisory lock 内取号 + 创建（同锁同事务，避免 race window）；号池匹配 `^{prefix}[0-9]{1,15}$` 取 max+1，位数溢出（如 3 位配到 1000）后仍单调递增。
- 号池查询在正则之外再加 `name LIKE '{prefix}%' ESCAPE '\'`（正则匹配蕴含前缀匹配，故不漏号；编号规则的 `_` 分隔符会被转义）。目的是命中 `products_tenant_id_name_pattern_idx` / `dev_styles_tenant_id_name_pattern_idx`（`varchar_pattern_ops`，migration `20260726010000_product_code_pool_pattern_indexes`）——库 collation 为 `en_US.UTF-8`，默认 opclass 的 `(tenant_id, name)` 索引无法用于 LIKE 前缀。锁内耗时直接决定同前缀取号的排队时间，故此处按索引扫描优化。
- 号池的**数字位数上限 15 位**（`PRODUCT_CODE_SERIAL_MAX_DIGITS`）。品名可手工录入任意长数字串（如粘贴条码 `SW-` + 20 位），不设上限时 19 位以上会让号池 SQL 的 `CAST(... AS BIGINT)` 抛 22003，导致该前缀家族的取号整体 500；16~18 位虽能转 BIGINT 但超出 JS 安全整数范围，`max + 1` 可能因精度丢失取到重复号。规则可配的 `serialLength` 上限为 10，15 位已留足溢出空间；被排除的超长编号不参与 max，仍受 `(tenant_id, name)` 唯一约束保护。
...
| `tenant_memberships.preferences` | `dashboardNotificationReadIds` | `string[]`（最多 300） | 工作台消息中心已读 id；网页/小程序共用，经 `GET/POST /api/dashboard/notification-reads` 同步 |
...
interface FinanceRecord {
  ...
  /** PSI 来源单据号（采购订单/采购入库/销售订单/销售单）；快捷登记收付款时写入，详情页反查已收付金额 */
  sourceDocNo?: string;
...
`sourceDocNo` 由 migration `20260727120000_finance_record_source_doc_no` 增加列与 `(tenant_id, source_doc_no)` 索引；列表可用 `GET /api/finance/records?sourceDocNo=` 精确过滤。
...
`DevStyle.defaultStageNames`（Json 字符串数组）：款式创建时配置的默认开发流程节点名。创建款式不再自动建头样；新增首个样品（头样）时带出这套默认节点。开发域 `DevStyle` / `DevSample` / `DevBom` / `DevStageTemplate` / `DevLog` 均有 `createdByUserId`（创建时后端写入，本阶段仅关联，见 `docs/01` §5.6.2）。`DevStyle.imageThumb`：列表缩略图（由 `imageUrl` 生成）；`GET /dev/styles` 列表 omit 原图与节点附件/文件字段二进制；详情 `GET /dev/styles/:id` 对节点 `type=file` 仅用 `LEFT/RIGHT(value)` 取文件名元数据生成 deferred stub（不把 data URL 读进 Node），`attachments.fileUrl` 置空；内容按需 `GET /dev/styles/stage-fields/:fieldId`（整字段 JSON）、`GET /dev/styles/stage-fields/:fieldId/files/:index`（单文件二进制 + Range，视频预览优先）、`GET /dev/styles/attachments/:attachmentId`。归档 / 生成商品回包同一套轻量装载；发布过程不 include 样品节点文件。
...
**开发领退料**：`reason = 来自于开发`（常量 `PROD_OP_REASON_FROM_DEV`），`customData.devStyleId` 关联款式；无 `orderId`/`partner`。生产物料统计须排除该来源；库存聚合计入。整单编辑/删除 DTO：`DevMaterialDocUpdateRequest` / `DevMaterialDocMutationResult`（`shared/types.ts`）。**返工领退料**（领料 `STOCK_OUT` / 退料 `STOCK_RETURN`）`reason = 来自于返工`（`PROD_OP_REASON_FROM_REWORK`），带 `orderId`、无 `partner`，同级隔离；生产物料「领料退料流水」类型标注「返工领料 / 返工退料」，仓库流水不变。DTO 见 `shared/types.ts` `ReworkMaterialRecordsResponse` 等（复用 `DevMaterial*` 形状）。
