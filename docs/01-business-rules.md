...
| 计划级覆盖 | 可在计划详情内增删、排序工序，保存至 `PlanOrder.milestoneNodeIds`；**不回写**产品档案；**关联产品模式**（`productionLinkMode=product`）下不展示「修改工艺路线」，统一使用产品标准路线 |
| 清除覆盖 | 调整后的路线与产品标准路线完全一致时，保存清除计划覆盖（存 `null`），继续沿用产品默认 |
| 下达工单 | `convertPlanToOrders` 优先使用计划有效路线（计划覆盖非空 → 计划，否则 → 产品）生成工单 `milestones` |
