...
- **资料库**（`knowledge_base` 插件）：租户内共享文件夹与文档；左侧树形管理（文档/文件夹可拖拽排序与移动），右侧 Tiptap 块级富文本（左侧 + 菜单插入标题/列表/待办/表格/代码块/图片/**文件或视频**/**关联产品**/**关联文档**/超链接等；超链接经弹窗填写文本与 URL）自动保存；图片经 `/api/knowledge-base/assets` 独立存储（不支持 SVG，≤10MB）；**附件**（任意文件，≤30MB；PDF / Excel `.xls/.xlsx` 可在线预览，**视频**可系统内播放，其余类型打开后提示无法预览并可下载）经同一 `/assets` 接口存储，正文内以飞书式附件卡片（`div[data-type=file-attachment]` + `data-asset-url`/`data-file-name`/`data-mime-type`/`data-size-bytes`，视频可选 `data-display-mode=player`）保存；视频默认**标签展示**，编辑时可切换为**内嵌视频窗口**（类似图片）；点击标签或眼睛图标全屏预览/播放（PDF iframe、Excel 多 Sheet 表格——`.xlsx` 经 ExcelJS 解析，还原合并单元格/列宽行高并按锚点渲染内嵌图片，`.xls` 旧格式回退 SheetJS 无图片、图片放大、视频 `<video controls>`；不可预览类型显示「.xxx 文件类型无法预览」占位并可下载），卡片与预览壳均可下载原始文件（下载走 `?download=1`，`Content-Disposition` 保留中文名）；**表格内插入图片默认按缩略图宽度展示，可拖角调整大小，点击预览原图**；**关联产品**以正文内联芯片保存（`span[data-type=product-ref]` + `data-product-id`），**点击弹出产品快览信息**（分类/颜色尺码/工序/BOM 等）；**关联文档**以正文内联芯片保存（`span[data-type=document-ref]` + `data-document-id` + `data-label`），可从资料库树选择其他文档（不可关联当前文档），**点击打开文档预览**；正文保存 HTML 白名单消毒；树/搜索接口不含正文，编辑时按文档 id 拉取；保存可带 `expectedUpdatedAt` 乐观锁；删文档前检查产品/开发款引用，仍被引用则 409；侧栏支持服务端搜索；侧栏入口位于「基础信息」上方；权限 `knowledge_base:folders:*` / `knowledge_base:documents:*`。
