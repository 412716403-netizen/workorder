...
**编辑采购订单与关联稳定性**：采购入库引用订单行时写入的 `sourceLineId` 对应订单行 PsiRecord 的 `id`。编辑采购订单做整单替换保存时，**须复用原行 `id`**（按 `sourceRecordIds` + 规格匹配），不得一律生成新 id，否则已生成的采购入库会断链、已入库进度归零。新增明细仍用新 id。
...
| 产品编号 | `{元素段…}{分隔符}{流水号}` | 按「产品编号规则」自动生成，见下节 |

#### 产品编号自动生成规则

- 规则**按产品分类各配一套**，存租户配置 `productCodeRules`（分类 id → `ProductCodeRule`，类型见 `shared/types.ts`）；未配置的分类为手动输入。
- 配置入口：产品新增/编辑表单与开发管理「创建开发款式」表单「产品编号」旁的齿轮按钮（需 `settings:config:edit` 权限）。
- 元素条数由用户自定义增删（1~10 条，新建默认 2 条），每条可选 **固定文本**（如 `WL`、`01`）或 **产品字段**：
  - 产品名称（`sku`）与 `text` 型分类扩展字段 → 取文本内容，可配截取前 N 位；
  - `select` 型扩展字段 → 「文本内容」（可配截取前 N 位）或「选项对应编号」（选项 → 编号映射，未映射的选项该段留空）；
  - `date` 型扩展字段 → 按格式输出：`yyMMdd`(260710) / `yyMM` / `yy` / `yyyyMMdd`；
  - **合作单位**（`partner`）：仅当该分类开启「关联合作单位」（`linkPartner`）时出现在字段下拉中，取产品档案「合作单位」（`Product.supplierId`）对应的**名称**，按 `select` 型处理 —— 可选「文本内容」（可截取前 N 位）或「选项对应编号」（以租户合作单位名称为选项；映射弹窗常驻搜索，并按合作单位分类 Tab 筛选，交互对齐合作单位选择器）。映射以**名称**为键，合作单位改名后需重新配置；分类关掉 `linkPartner` 或产品未关联合作单位时该段留空。
- 非空段用分隔符（`-`/`_`/`/`/无）连接，空段跳过；流水号（1-10 位左补零）恒为最后一段。
- **流水号按前缀分组**：除流水号外拼接结果相同的产品共用一个号池（`WL-棉线-001/002`；`WL-化纤-001` 从 1 重新计）。号池取同前缀下的最大数字后缀 +1，超过 15 位的数字后缀（手工录入的条码等）不计入，详见 `docs/02` §1.5.1。
- 生成时机：新建产品打开表单即预取号填入（可手改、可点刷新重取）；保存时若未手改，后端在 `product_code:{prefix}` advisory lock 内**重新取号 + 创建**，并发不重号（最终号可能与预览号不同）。手改过则按用户输入保存并查重。
- **开发管理**「创建开发款式 / 录入新产品」同样按分类规则预取号写入品名（产品编号）；配置入口仍在产品档案表单齿轮。预取号权限：`basic:products:create` 或 `development:styles:create`。号池同时覆盖 `products.name` 与 `dev_styles.name`。
- 批量导入（`ProductImportModal`）不走该规则，仍按导入文件的编号列。
...
- **产品经营情况**：工作台提供两个独立组件——**产品经营·报工耗材**（`product_economics_consumable`）与 **产品经营·单据关联**（`product_economics_document`），可分别添加到页面；**物料成本口径由所添加的组件决定**，系统设置中不再提供全局切换；旧 `product_economics` 自动视为报工耗材组件。详见 `docs/02-data-structures.md` §1.6。
- **报工耗材物料单价**：成品 BOM 默认按**最近一次采购价**统计（可改为自定义时间区间）；可在 **更多 → 物料价格** 按成品分级设定，并对单条物料覆盖。存 `products.economicsBomMaterialPrice`。
...
- **平台管理员**（`users.role === admin`）：侧栏仅「信息发布」「账号管理」；登录默认进入 `/announcements`；不可访问 ERP 业务模块。账号管理内含「使用情况」Tab，可查看各企业业务用量与系统负担（单品码、报工流水、资料库字节等），见 [`07-auth-tenant-session.md`](./07-auth-tenant-session.md) §4.5.1。
...
- **资料库**（`knowledge_base` 插件）：租户内共享文件夹与文档；左侧树形管理（文档/文件夹可拖拽排序与移动），右侧 Tiptap 块级富文本（左侧 + 菜单插入标题/列表/待办/表格/代码块/图片/**文件或视频**/**关联产品**/**关联文档**/**关联单据**/超链接等；超链接经弹窗填写文本与 URL）自动保存；图片经 `/api/knowledge-base/assets` 独立存储（不支持 SVG，≤10MB），**正文图片进入视口附近才请求**（长文档不再一次性拉全部图片）；插入时按原图记录 `width`/`height`（正文超出编辑区宽度则等比缩，表格内固定缩略图宽度并换算等比高），历史图片缺高度时按占位高撑开版面，保证未加载前不塌高；**附件**（任意文件，≤30MB；PDF / Excel `.xls/.xlsx` / Word `.docx` 可在线预览，**视频**可系统内播放，其余类型打开后提示无法预览并可下载）经同一 `/assets` 接口存储，正文内以飞书式附件卡片（`div[data-type=file-attachment]` + `data-asset-url`/`data-file-name`/`data-mime-type`/`data-size-bytes`，视频可选 `data-display-mode=player`）保存；视频默认**标签展示**，编辑时可切换为**内嵌视频窗口**（类似图片）；**内嵌窗口打开文档时不预拉视频字节，需点击「加载视频」后再请求**（全屏预览仍按用户操作加载）；点击标签或眼睛图标全屏预览/播放（PDF iframe、Excel 多 Sheet 表格——`.xlsx` 经 ExcelJS 解析，还原合并单元格/列宽行高并按锚点渲染内嵌图片，`.xls` 旧格式回退 SheetJS + BIFF 锚点抽图（`excelXlsGrid`，抽图与解析并行、尺寸 `Promise.all`）、图片放大、视频 `<video controls>`、Word `.docx` 经 docx-preview 分页渲染（页边距/图片位置比纯 HTML 更接近 Word，复杂浮动排版仍可能有差异），旧版 `.doc` 提示下载；不可预览类型显示「.xxx 文件类型无法预览」占位并可下载），卡片与预览壳均可下载原始文件（下载走 `?download=1`，`Content-Disposition` 保留中文名）；**表格内插入图片默认按缩略图宽度展示，可拖角调整大小，点击预览原图**；表格支持拖选多单元格，浮动工具栏可合并/拆分单元格并批量设置字体样式与对齐；**关联产品**以正文内联芯片保存（`span[data-type=product-ref]` + `data-product-id`），**点击弹出产品快览信息**（分类/颜色尺码/工序/BOM 等）；**关联文档**以正文内联芯片保存（`span[data-type=document-ref]` + `data-document-id` + `data-label`），可从资料库树选择其他文档（不可关联当前文档），**点击打开文档预览**；**关联单据**以正文内联芯片保存（`span[data-type=biz-doc-ref]` + `data-doc-kind`/`data-doc-id`/`data-doc-number`/`data-label`），可选生产计划 / 采购入库 / 销售单，**点击打开对应业务详情（只读，顶栏保留「待办」）**；正文保存 HTML 白名单消毒；树/搜索接口不含正文，编辑时按文档 id 拉取；保存可带 `expectedUpdatedAt` 乐观锁；删文档前检查产品/开发款引用，仍被引用则 409；侧栏支持服务端搜索；侧栏入口位于「基础信息」上方；权限 `knowledge_base:folders:*` / `knowledge_base:documents:*`。
...
- **消息 feed**：聚合平台公告、到期提醒；无独立 Notification 表。无其它消息时展示占位「欢迎使用工作台」，`createdAt` 取成员加入企业时间（稳定发送时间，非每次拉取的当前时刻）；软件到期提醒的 `createdAt` 取提醒当日 0 点。
- **消息已读**：按用户落在 `tenant_memberships.preferences.dashboardNotificationReadIds`（上限 300，新 id 优先保留）；`GET/POST /api/dashboard/notification-reads` 供网页与小程序同步。两端本地缓存作乐观更新，进入消息页时先拉服务端再渲染未读角标；客户端只上报本地新增 id，服务端合并后无变化则跳过写库。
...
| 类型 | 格式 | 规则 |
|------|------|------|
| 计划单号 | `PLN1`, `PLN2`, ... | 与工单号**共用主序号池**：取 `max(计划主序号, 工单主序号) + 1`；协作接单现先建计划占 `PLNn` |
| 子计划单号 | `PLN1-S1`, `PLN1-S2`, ... | 从父计划派生；多级继续追加 `-S{序号}` |
| 拆单计划号 | `PLN1-1`, `PLN1-2`, … `PLN1-99` | 从同一成品计划按数量拆出独立计划（无 `parentPlanId`）；可对已拆单继续拆（如 `PLN1-1-1`）；与子计划 `-S` 后缀无关 |
...
- 有效工序路线为空时禁止下达（须先在计划详情配置工序）。
- 若目标号已被**无 `planOrderId` 的历史协作孤儿工单**占用，且**产品一致** → 挂接该工单（写入 `planOrderId` 等），不新建第二条（兼容旧接单建工单数据）。
- 协作接单计划下达成功后，将工单 id 回填至对应派发的 `receiverProductionOrderId`。
- 若目标号已被其他计划占用，或已被不同产品工单占用 → 返回 409。
- 历史已存在的不一致编号不做自动迁移。
...
| 规则项 | 说明 |
|--------|------|
| 开关 | `planFormSettings.listDisplay.splitPlanEnabled`；开启后计划详情底部显示「拆单」 |
| 操作 | 每次提交仅创建 **1** 条新计划；用户填写**拆到新单**的数量，原单扣减剩余量 |
| 约束 | 未下达（无关联工单、`status !== CONVERTED`）；非 BOM 子计划；拆出后原单至少保留 1 件 |
| 单号 | `POST /api/plans/:id/split` → 新单 `{源单号}-{1..99}` 取最小可用序号 |
| 列表 | 同根单号（反复去末尾 `-数字`）≥2 条时在计划列表「原单 X 拆分」分组展示 |
| 与子计划 | 拆单不写 `parentPlanId`；子计划（物料 BOM 层级）仍走 `POST /api/plans/:id/sub-plans` |

### 3.3.2 计划单入单时间

| 规则项 | 说明 |
|--------|------|
| 字段 | 业务「入单时间」，持久化至 `PlanOrder.createdAt`（**仅日期** `YYYY-MM-DD`） |
| 默认 | 新建默认当天；Web 新建/编辑与小程序新建均可修改 |
| 落库 | 提交前转为本地日历日 0 点 ISO（`localCalendarYmdStartToIso`），与进销存日期口径一致 |
| 列表 | 计划列表仍按 `createdAt` 倒序；修改入单时间后排序随之变化 |

### 3.3.3 全单据入单时间（创建时间）

| 规则项 | 说明 |
|--------|------|
| 含义 | 业务「入单时间」写入各单据**现有**时间字段，不新增 DB 列 |
| 粒度 | **仅日期**（`YYYY-MM-DD`）：进销存 PO/SO/PB/SB、调拨、盘点、计划单 → `createdAt`；**日期+时间**：财务、工单、生产流水 → `timestamp`（工单为 `createdAt`） |
| 默认 | 新建 = 当前时刻（日期类默认今天） |
| 编辑 | 回填原单时间，允许修改后保存 |
| 落库 | 日期类用 `localCalendarYmdStartToIso`；datetime 类用 `entryDatetimeLocalToTimestamp`；进销存 `createdAt` 与 `timestamp` 同步 |
| 排除 | 小程序报工；Web 报工弹窗保持自动时刻 |
...
**例外**：协作乙方 `acceptTransfer` 接单自动创建的生产计划允许产品暂无工序；须在计划详情配置工序（并可写回产品）后再下达工单。

工单中心「创建生产计划」弹窗支持两种入口：

| 入口 | 说明 |
|------|------|
| 直接手动创建 | 与历史行为一致，手工选产品与客户并录入数量 |
| 引用销售订单生成 | 需 `psi:sales_order:view`；列出仍有**未配货**数量的销售订单；数量口径为 `订货量 − 已配`；一单多品项时需先选品项行；`PlanOrder.customData.sourceSalesOrderDocNumber` 记录来源单号，**计划详情与列表均展示「来源销售订单」**；表单配置开启「客户」且非产品关联模式时，自动带入销售订单合作单位为客户；**同一销售订单下已为该产品建过引用计划时，再次引用不再展示该品项**；保存前仍可调整数量与客户；**销售订单列表「操作」列**：已引用建计划的品项行展示「计划详情」/「工单详情」，**在当前页弹窗打开**（未下达 → 计划详情；已下达 `CONVERTED` → 关联工单详情），与计划列表操作区口径一致；**工单详情弹窗**在「工单明细」下方展示与工单中心列表相同的**工序进度小卡**（可报/已报/剩余） |

### 3.7.1 计划单工序路线

| 规则项 | 说明 |
|--------|------|
| 默认展示 | 计划详情「工序任务」默认展示关联产品 `Product.milestoneNodeIds` 标准生产路线 |
| 计划级覆盖 | 可在计划详情内增删、排序工序，保存至 `PlanOrder.milestoneNodeIds`；**一般不回写**产品档案；**关联产品模式**（`productionLinkMode=product`）下，产品已有工序时不展示「修改工艺路线」 |
| 产品无工序 | 两种模式下均可在计划详情「配置工艺路线」；弹窗确认后立即保存，若产品原先无工序则**写回** `Product.milestoneNodeIds`（仅此首次场景） |
| 清除覆盖 | 调整后的路线与产品标准路线完全一致时，保存清除计划覆盖（存 `null`），继续沿用产品默认 |
| 下达工单 | `convertPlanToOrders` 优先使用计划有效路线（计划覆盖非空 → 计划，否则 → 产品）生成工单 `milestones`；有效路线为空则拒绝下达 |
| 锁定 | 存在 `planOrderId` 指向本计划的关联工单时，禁止修改工序路线（与数量明细锁定口径一致）；删光关联工单后可再次编辑 |
| 拆单 | 拆出的新计划继承源计划的 `milestoneNodeIds` 覆盖（若有） |
...
回退后计划单派生状态为「未下单」，可再次「下达工单」，详情中的数量明细、工序路线与 BOM 汇总恢复可编辑。
...
计划单列表**默认显示采购订单进度**（汇总到货百分比 + 迷你进度条，不展示物料明细；无关联采购订单的行不显示）。

计划单表单配置 **列表显示** 页签另有「计划单拆单」（`planFormSettings.listDisplay.splitPlanEnabled`）：开启后计划详情底部显示「拆单」按钮；每次从当前计划拆出 1 条新计划单（详见 §3.3.1）。
...
### 4.2.3 PSI 单据快捷登记收付款单

- **入口**：进销存四单（采购订单 / 采购入库 / 销售订单 / 销售单）的**新增/编辑表单**合计金额条右侧「登记付款单」或「登记收款单」按钮（需对应 `finance:payment:create` / `finance:receipt:create`）。列表卡片头不放此按钮；详情页只展示已关联金额，不提供登记入口。
- **方向映射**（`PSI_DOC_FINANCE_OP_TYPE`）：采购订单、采购入库 → 付款单（`PAYMENT`）；销售订单、销售单 → 收款单（`RECEIPT`）。
- **预填**：合作单位带入当前单据 `partner`（**不可改**）；备注默认 `关联{单据类型} {单号}`。**须先选择合作单位**后才可点「登记收/付款单」（未选时按钮禁用并提示）；新增态单号取表单预览号（选单位后即生成），编辑态取已保存单号。**金额留空**；单据分类 / 关联产品不预填（选中开启「关联产品」的分类后才显示产品字段，默认空）。
- **新增态暂存**：单据尚未保存时登记的收付款先暂存在表单（`OrderBillFormPage.stagedFinanceDrafts`），合计条实时显示已登记金额；点「保存单据」取得正式单号后一并 `POST /api/finance/records`，并写入 `sourceDocNo`。落库失败只 toast 告警，不回滚已保存的 PSI 单据。
- **编辑态**：有单号时直接创建收付款单并写入 `sourceDocNo`。
...
| 子模块 | 管理实体 | 规则 |
|--------|----------|------|
| 产品分类管理 | `categories` | 支持 `customFields` 扩展；**名称租户内唯一**（新增/编辑，忽略首尾空白与大小写）；左侧列表支持拖拽调整 `sortOrder`（`PUT /settings/categories/reorder`，需 `settings:categories:edit`），各选择器按该顺序展示；**「启用颜色尺码」开关仅毛衣工厂行业租户可见**（租户 `industryKind = sweater_factory`，由平台在企业管理中指定，经登录/选企业/租户列表接口透传到前端 `tenantCtx.industryKind`）；通用行业租户隐藏该开关，但已开启颜色尺码的分类仍显示开关便于关闭；**删除限制**：被产品（`Product.categoryId`）或开发款式（`DevStyle.categoryId`）引用时禁止删除，`deleteCategory` 返回 409 列明细；前端经 `GET /settings/categories/usage` 预检，被引用项删除按钮置灰并提示 |
...
- **小程序 Tab「报工」自报工审核**（`pages/scan`）：
  ...
  - **报工链路只读依赖放宽**（`requireSubPermissionOrProductionRead`，`backend/src/middleware/tenant.ts`）：产品列表/详情/变体（`/products`）、产品分类与工序节点（`/settings/categories`、`/settings/nodes`）、字典与设备（`/master/dictionaries`、`/master/equipment`）、单品码/虚拟批次**扫码解析与追溯**（`/item-codes/scan|trace`、`/plan-virtual-batches/scan|trace`、`validate-usage`）这些 GET/只读端点，持有原细粒度键 **或生产域任一权限**（裸 `production`、任意 `production:*`、`process_report`）即可读取——否则仅有工序报工权限的工人会出现报工页无颜色尺码矩阵、工序名显示为节点 id、扫码录入 403。
  ...
  - Tab 手输提交带 `requireApproval: true` → 落库 `approvalStatus=PENDING`；**不计入** `completedQuantity` / 顺控上游，但 **占用可报额度**（下次可报须扣 PENDING）。**工单中心工序卡**圆心与下方可报数字不扣待审（与 Web 一致）；报工详情 hint 单独展示「待审 N」。报工单号前缀 **`ZBG`**（自报工专用），与工单中心即时报工的 **`BG`** 分池取号，避免重号。
  ...
  - 审核：`production:orders_report_records:edit`；Web 入口在「报工流水」弹窗标题栏右侧「报工审核」（样式对齐待入库清单内「入库流水」），不再单独占工具栏按钮；有待审时工具栏「报工流水」显示 `报工流水（N）`（N 按 `reportBatchId` 聚合，与待审列表行数一致）；小程序仍为「报工待审」。**通过后才进入报工流水**；流水不展示审核状态；驳回后释放额度。Web 报工审核弹窗可叠在流水之上（z-index 更高），**默认不限日期**（展示全部待审），可选手动收窄日期；支持勾选**批量通过/驳回**。
...
- **乙方自动建生产计划与编号**：`acceptTransfer` 接受派发时自动创建 `PlanOrder`（**不经工单中心**），并写入派发 `receiverPlanOrderId`。拆分规则与原先建工单一致：**产品模式**整条转单共用 1 张计划；**工单模式**每个派发各 1 张计划。计划号走与工单共用的主序号池（`getNextPlanNumber`）。产品可暂无工序（`pendingProcess`）；须在计划详情配置工序后再「下达工单」。下达成功后回填 `receiverProductionOrderId`。**回传**要求相关派发已有工单 id，否则提示先下达。甲方修订数量：未下达时同步 `PlanItem`；已下达时同步工单明细并可同步计划。历史「接单直接建孤儿工单」数据仍可通过计划下达时的同号挂接兼容。
...
| 关联合作单位 | `linkPartner` | 产品档案 / 产品详情的「合作单位」字段（存 `supplierId`，可选**任意分类**的合作单位）；产品档案列表、生产管理（计划/工单/外协/返工/领退料）与进销存列表在产品编号下的元信息行展示该名称并支持按它搜索；商品选择组件（`SearchableProductSelect`）在行内展示该名称并支持按它搜索；开发管理按客户排序/搜索由该合作单位名称推导 |
| 互斥 | — | 已启用采购价时不可单独关闭「关联合作单位」 |

**开发管理按客户排序**：左侧「按客户」由款式关联的合作单位（`supplierId` → 合作单位名称）分组；`customerName` 在保存时自动同步，兼容历史数据。须至少一个产品分类启用 `linkPartner`。

**开发管理左侧列表卡片**：首行「产品编号」（`name`）后紧跟「产品名称」（`code`）；下一行展示合作单位（分类开启 `linkPartner` 时）与分类自定义字段标签（与产品档案列表同口径，不含附件类）。

**款号 / 品名唯一性**：创建或编辑开发款式时，`name`（品名，对应产品编号）必填且须在租户内唯一——不得与已有 **产品档案** 或其它 **开发款式** 的品名重复；`code`（款号，对应产品名称/`sku`）**选填、不自动生成**，且**允许租户内重复**——既可与产品档案中的产品名称重复，也可与其它开发款式的款号重复（与 `products.sku` 同口径，见 migration `20260728120000_dev_style_code_drop_unique`）。**发布大货**同样不强制款号：为空时生成的产品 `sku` 存 NULL，单 SKU 变体的 `skuSuffix` 为空串（与产品档案新建/导入同口径）。
...
- 已发布款式的 `publishedProductId` 指向产品档案。
...
- **领料**：仅 `developing` 款式可领；可领范围为该款试制 BOM 顶层物料（按 `productId` 去重）**及其产品档案 BOM 下级物料**（前端汇总表 / 领料弹窗可逐级展开，深度上限 `DEV_MATERIAL_BOM_MAX_DEPTH = 8`，环路按路径截断）；扣减仓库库存，启用批次管理时须选批号。领料界面展示「单个用量」列：顶层取试制 BOM 用量，展开子料取产品档案 BOM 相对父件用量（跨变体/工序去重时保留首次用量）。
...
- **图片/文件字段可上传多个**（最多 9 个，**不限文件类型**）：存 JSON `[{url,name},...]`（保留原始文件名）；历史单文件裸 data URL / URL 数组仍兼容。网页端与小程序节点登记一致；下载使用原文件名。小程序单文件约 ≤4MB。
- **在线预览**：网页端复用资料库 `KnowledgeFilePreviewOverlay`（PDF iframe / Excel / Word / 图片）；附件仍为表单内 data URL，不迁入资料库资产表。旧版 `.doc`、加密 Office 等可能无法预览，可下载。小程序：图片走 `wx.previewImage`；PDF / Word / Excel 写临时文件后 `wx.openDocument`，失败则提示在电脑端查看。
...
- **微信服务号推送（可选）**：用户在工作台待办面板「微信提醒」或小程序「我的 → 微信提醒」扫带参二维码关注服务号后，系统将 `users.wx_mp_openid` 与账号绑定。**小程序消息模块三类来源**均会推送（对齐消息 Tab）：① 消息中心——平台公告（发布时广播 + 轮询兜底）、租户到期提醒（到期前 7/3/1 天）；② 待办事项——`remindAt` 到点的待办；③ 协作——待接受派发 / 待收回回传 / 待确认转发。后端约每 60s 扫描一次，发送记录写入 `wx_push_logs`（按 `dedupe_key` 幂等）。当前统一使用服务号模板 `todo_remind`（环境变量 `WX_MP_TEMPLATE_TODO_REMIND`，字段 thing1+time3）；点击分别跳转小程序消息会话页。未绑定用户不推送；取消关注或主动解绑会清空 `wx_mp_openid`。回调地址：`GET/POST /webhooks/wechat-mp`（验签，无 JWT）。
