...
- **统一下发计划单列表 / 单品码与批次码标签**：读配置时合并 **`builtin-plan-list-v1`**（A4，`planList`）、**`builtin-plan-label-v1`**（30×50mm，`planItemLabel`，单品码行占位符）与 **`builtin-plan-batch-label-v1`**（30×50mm，`planBatchLabel`，`{{批次.*}}` 虚拟批次标签）；与其它锁定内置模版相同：列表标「系统」、不可删、不可直接可视化编辑保存，可复制为自有模版；持久化规则同上。生产计划单表单配置「打印模版」中标签打印拆为 **单品码打印**（`labelPrint.itemCodePrint.allowedTemplateIds`）与 **批次码打印**（`labelPrint.batchPrint.allowedTemplateIds`）；历史 `labelPrint.allowedTemplateIds` 整份迁移至单品码白名单，历史 `planLabel` 模版 scope 归一化：批次内置 → `planBatchLabel`，其余（含用户自建）→ `planItemLabel`。若租户单品码白名单误只含列表模版，`repairPlanLabelPrintWhitelistMissingPlanLabelTemplates` 会在加载时并入所有 `planItemLabel` 模版 id。
