...
生产各子模块的「表单配置」按钮按 `production:*_form_config:allow` 控制，但配置持久化在 `system_settings`（`GET/PUT /settings/config`）。

- **读**：`settings:config:view`，或持有任一 `production:*_form_config:allow`（外协为 `production:outsource_form_config:allow`）。
- **写**：`settings:config:edit`，或写入的 key 与对应 `production:*_form_config:allow` 匹配（如 `outsourceFormSettings` ↔ 外协表单配置）。
