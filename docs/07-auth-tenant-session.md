...
- 全局 `role=admin` 账号通过 **`PUT /api/admin/tenants/:id`** 可维护企业状态、到期时间、设备模块开关，以及 **`industryKind`**（`generic` | `sweater_factory`，见 `shared/types.ts`）。
...
- 普通租户成员 JWT 不依赖该字段；行业类型仅用于平台侧初始化与展示。
