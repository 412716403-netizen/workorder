...
- **真源**：`Tenant.productionLinkMode`（Prisma `tenants.production_link_mode`）
- **镜像**：`system_settings` 中同名键，平台保存时同步写入，供历史读路径与前端 config 加载兼容
- 后端 `getProductionLinkMode(tenantId)` **优先读 Tenant 字段**，无记录时 fallback `systemSetting`

### 4.3 配置归属（平台化管理）

| 角色 | 能否配置 | 入口 |
|------|----------|------|
| 平台管理员 | 是 | 账号管理 → 企业管理（审核通过 / 编辑租户） |
| 租户成员 | **否** | 生产业务配置页**已移除**该选项；`PUT /settings/config/productionLinkMode` 返回 **403** |

规则：

- **审核通过（pending → active）时必须选择**「关联工单」或「关联产品」
- 开通后模式立即生效；各业务页从 config 只读消费，行为与原先一致
- **一旦有生产业务数据则锁定**：平台不可再改模式（409），前端展示只读说明

### 4.4 锁定判定

满足**任一**即视为已有生产数据、`productionLinkMode` 不可变更：

- 存在 `MilestoneReport` 或 `ProductProgressReport`
- 存在 `ProductionOpRecord`，且 `type` ∈ `OUTSOURCE | REWORK | REWORK_REPORT | SCRAP | STOCK_IN | STOCK_OUT | STOCK_RETURN`
- 存在 `Milestone` 且 `completedQuantity > 0`

实现见 `backend/src/utils/tenantProductionActivity.ts`。

### 4.5 变更原则

- **不做模式切换时的数据迁移**；靠「开通时选定 + 有数据后锁定」从源头避免 PMP / milestone 混用
- 配置变更后，优先影响**新产生的数据**
- 历史数据不应被隐式重写
...
| `OrderListView` 产品组「详情」→ `ProductProductionDetailModal` | `aggregateProductReportSummaryByNode`（PMP + milestone 双路良品/不良；SCRAP 按 productId） | 外协 `aggregateProductOutsourcePartners`（发出/已收回/剩余） | 读口径与产品组列表卡、外协管理产品模式一致 |
| `OrderListView` 工单流水「详情」→ `OrderDetailModal` + `detailFromFlowLayout` | 单工单 milestone 良品/不良；SCRAP 按 `orderId` | 外协按工单 `orderId` 聚合（发出/已收回/剩余） | 与产品模式工单流水单工单详情布局一致；不展示报工汇总表与客户/交期 |
...
自平台化管理方案起，**租户侧不再提供模式切换 UI**（`ProductionConfigTab` 仅保留超报、外协超收、称重容差等可配项）。

- 开通时由平台管理员在「企业管理」选定模式
- 产生生产数据后锁定，平台亦不可再改
- 若历史租户曾混用两种模式报工，展示层问题需运营侧约定口径或人工处理；**本期不做**切换迁移与技术层双路合并修复
