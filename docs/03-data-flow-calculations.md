...
- 若此前已用「纯计划单品码」（`batchId` 为空）生成过码，又在同一计划、同一规格下创建虚拟批次并勾选「同时生成单品码」，则该规格下旧的纯计划单品码会在同一事务内被标记为 `VOIDED`（列表默认只查 `ACTIVE`，不再展示；旧标签扫码提示已作废）
- 计划单详情「追溯码」仅提供「批次码」与「单品码+批次码」两种生成入口，不再提供「仅单品码、不经批次」的生成按钮（后端 `POST /item-codes/generate` 仍保留，供脚本或将来其它入口使用）
- 计划详情「一键生成全部规格」的默认每批件数存于租户 `planFormSettings.labelPrint.bulkQuickSplitBatchSize`；仅在计划详情「追溯码」中通过「拆批设置」维护；未配置有效每批件数时一键生成不可用。是否同步生成单品码跟随详情页「生成类型」（「单品码+批次码」同步生成，「仅批次码」不生成），不再单独勾选；历史字段 `bulkQuickSplitWithItemCodes` 归一化恒为 true。
