...
- 计划详情「一键生成全部规格」的默认每批件数、以及（在「单品码+批次码」模式下）是否同步生成单品码，存于租户 `planFormSettings.labelPrint` 的 `bulkQuickSplitBatchSize` / `bulkQuickSplitWithItemCodes`；仅在计划详情「追溯码」中通过「拆批设置」维护；未配置有效每批件数时一键生成不可用
