...
import { useMaterialPriceSettings } from '../../hooks/useMaterialPurchasePrices';
import {
  useOutsourcePriceNodes,
  useOutsourcePriceParentProducts,
  usePatchOutsourcePriceNodeOverride,
  usePatchParentOutsourcePriceDefaultRule,
} from '../../hooks/useOutsourceProcessPrices';
import {
  usePatchParentReportPriceDefaultRule,
  usePatchReportPriceNodeOverride,
  useReportPriceNodes,
  useReportPriceParentProducts,
} from '../../hooks/useReportProcessPrices';
import {
  PROCESS_NODE_PRICE_RULE_SOURCE_LABEL,
  PROCESS_PRICE_SOURCE_LABEL,
...
export type ProcessNodePriceVariant = 'report' | 'outsource';

const VARIANT_META: Record<
  ProcessNodePriceVariant,
  {
    title: string;
    description: string;
    defaultRuleTitle: string;
    saveDefaultToast: string;
    saveNodeToast: string;
    emptyParent: string;
...
> = {
  report: {
    title: '报工价格',
    description:
      '选择有标准生产路线的成品，设定各工序报工单价统计规则；默认按最近一次报工单价，也可改为自定义时间区间；单工序可单独覆盖。',
    defaultRuleTitle: '成品报工统计规则',
    saveDefaultToast: '已保存成品报工统计规则，单工序单独设置已同步清除',
    saveNodeToast: '已保存单工序规则',
    emptyParent: '暂无有标准路线的成品或未匹配结果',
    lastPriceLabel: '最近一次单价',
  },
  outsource: {
    title: '外协价格',
    description:
      '选择有标准生产路线的成品，设定各工序外协单价统计规则；默认按最近一次外协收回单价，也可改为自定义时间区间；单工序可单独覆盖。',
    defaultRuleTitle: '成品外协统计规则',
    saveDefaultToast: '已保存成品外协统计规则，单工序单独设置已同步清除',
    saveNodeToast: '已保存单工序规则',
    emptyParent: '暂无有标准路线的成品或未匹配结果',
    lastPriceLabel: '最近一次单价',
  },
...
interface ProcessNodePriceModalCoreProps {
  open: boolean;
  onClose: () => void;
  showAmount: boolean;
  onSaved?: () => void;
  variant: ProcessNodePriceVariant;
}

type ProductBomRuleMode = 'fixed_range' | 'last_purchase';

function nodeOverrideModeFromRow(row: ProcessPriceNodeRow): ProductBomRuleMode {
  if (row.hasIndividualOverride && row.ruleSource === 'node_override' && row.ruleLabel.includes('~')) {
    return 'fixed_range';
  }
...
function ProductNodeRuleEditor({
  parentDefaultRule,
  canEdit,
  saving,
  title,
...
const ProcessNodePriceModalCore: React.FC<ProcessNodePriceModalCoreProps> = ({
  open,
  onClose,
  showAmount,
  onSaved,
  ...
  const [selectedParent, setSelectedParent] = useState<ProcessPriceParentProductRow | null>(null);
  ...
  const reportParentQuery = useReportPriceParentProducts(debouncedSearch);
  const outsourceParentQuery = useOutsourcePriceParentProducts(debouncedSearch);
  const reportNodesQuery = useReportPriceNodes(
    variant === 'report' ? selectedParent?.productId ?? null : null,
  );
  const outsourceNodesQuery = useOutsourcePriceNodes(
    variant === 'outsource' ? selectedParent?.productId ?? null : null,
  );
  const patchReportParent = usePatchParentReportPriceDefaultRule(
    variant === 'report' ? selectedParent?.productId ?? null : null,
  );
  const patchOutsourceParent = usePatchParentOutsourcePriceDefaultRule(
    variant === 'outsource' ? selectedParent?.productId ?? null : null,
  );
  const patchReportNode = usePatchReportPriceNodeOverride(
    variant === 'report' ? selectedParent?.productId ?? null : null,
  );
  const patchOutsourceNode = usePatchOutsourcePriceNodeOverride(
    variant === 'outsource' ? selectedParent?.productId ?? null : null,
  );

  const parentQuery = variant === 'report' ? reportParentQuery : outsourceParentQuery;
  const nodesQuery = variant === 'report' ? reportNodesQuery : outsourceNodesQuery;
  const patchParentMutation = variant === 'report' ? patchReportParent : patchOutsourceParent;
  const patchNodeMutation = variant === 'report' ? patchReportNode : patchOutsourceNode;

  const canEditProduct = settingsQuery.data?.canEditProduct ?? false;
  const parentRows = parentQuery.data?.rows ?? [];
  const nodeRows = nodesQuery.data?.rows ?? [];
  const parentDefaultRule = nodesQuery.data?.parentDefaultRule ?? null;

  const priceSourceLabel = useMemo(
    () => (source: ProcessPriceNodeRow['priceSource']) =>
      source ? PROCESS_PRICE_SOURCE_LABEL[source] : '—',
    [],
  );

  const saveProductRule = async (rule: MaterialPriceRule) => {
    if (!selectedParent) return;
    try {
      await patchParentMutation.mutateAsync(rule);
      setEditingNodeId(null);
      toast.success(meta.saveDefaultToast);
      onSaved?.();
    } catch (e) {
  ...
  const saveNodeOverride = async (nodeId: string, override: MaterialPriceRuleOverride) => {
    try {
      await patchNodeMutation.mutateAsync({ nodeId, rule: override });
      toast.success(meta.saveNodeToast);
      setEditingNodeId(null);
      onSaved?.();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : '保存失败');
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="process-node-price-title"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 px-6 py-4">
          <div>
            <h2 id="process-node-price-title" className="text-lg font-black text-slate-900">
              {meta.title}
            </h2>
          ...
          {!selectedParent ? (
            parentQuery.isLoading ? (
              ...
              <p className="py-16 text-center text-sm text-slate-400">
                {parentQuery.isFetching ? '加载中…' : meta.emptyParent}
              </p>
            ) : (
              <table className="w-full table-fixed text-xs">
                ...
                <thead>
                  <tr className="border-b border-slate-100 text-[11px] text-slate-400">
                    ...
                    <th className="px-3 py-2 text-center font-medium">工序数</th>
            ...
            <div className="space-y-3 py-2">
              <ProductNodeRuleEditor
                parentDefaultRule={parentDefaultRule}
                canEdit={canEditProduct}
                saving={patchParentMutation.isPending}
                title={meta.defaultRuleTitle}
                lastPriceLabel={meta.lastPriceLabel}
                onSave={saveProductRule}
              />

              {nodeRows.length === 0 ? (
                <p className="py-12 text-center text-sm text-slate-400">该成品暂无标准路线工序</p>
              ) : (
                <table className="w-full table-fixed text-xs">
                  <colgroup>
                    {canEditProduct ? (
                  ...
                  <thead>
                    <tr className="border-b border-slate-100 text-[11px] text-slate-400">
                      ...
                      <th className="px-3 py-2 text-center font-medium">流水数</th>
                  ...
                  <tbody>
                    {nodeRows.map(row => (
                      <NodeRow
                        ...
                        canEdit={canEditProduct}
                        editing={editingNodeId === row.nodeId}
                        priceSourceLabel={priceSourceLabel}
                        lastPriceLabel={meta.lastPriceLabel}
                        saving={patchNodeMutation.isPending}
                        onBeginEdit={() => setEditingNodeId(row.nodeId)}
                        onCancelEdit={() => setEditingNodeId(null)}
                        onSaveOverride={rule => void saveNodeOverride(row.nodeId, rule)}
                      />
                    ))}
                  </tbody>
                </table>
...
function NodeRow({
  ...
  row: ProcessPriceNodeRow;
  showAmount: boolean;
  canEdit: boolean;
  editing: boolean;
  priceSourceLabel: (source: ProcessPriceNodeRow['priceSource']) => string;
  lastPriceLabel: string;
  saving: boolean;
  onBeginEdit: () => void;
  onCancelEdit: () => void;
  ...
  const [mode, setMode] = useState<ProductBomRuleMode>(nodeOverrideModeFromRow(row));
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');

  useEffect(() => {
    if (!editing) return;
    const nextMode = nodeOverrideModeFromRow(row);
    setMode(nextMode);
  ...
  return (
    <>
      <tr className="border-b border-slate-50 align-middle">
        <td className="truncate px-3 py-2.5 font-medium text-slate-800" title={row.nodeName}>
          {row.nodeName}
        </td>
        <td className="px-3 py-2.5 text-center tabular-nums">
          <div className="font-bold text-slate-800">
            {formatWorkbenchAmount(row.unitPrice ?? 0, showAmount && row.unitPrice != null)}
            <span className="ml-0.5 text-[10px] font-normal text-slate-400">/件</span>
          </div>
          {row.priceSource ? (
        ...
        <tr className="border-b border-violet-50 bg-violet-50/40">
          <td colSpan={canEdit ? 5 : 4} className="px-3 py-3">
            <div className="space-y-2">
              <div className="text-[11px] font-bold text-slate-600">单工序规则 · {row.nodeName}</div>
              <div className="flex flex-wrap items-end gap-3">
                {(
                  [
                    ['last_purchase', lastPriceLabel],
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {row.hasIndividualOverride ? (
                    <button
                      type="button"
                      disabled={saving}
                      onClick={() => onSaveOverride({ inherit: true })}
...
export default ProcessNodePriceModalCore;
