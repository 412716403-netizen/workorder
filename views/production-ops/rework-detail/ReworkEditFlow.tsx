/**
 * 返工/返工报工流水 - 编辑视图 (P9 抽离自 ReworkReportFlowDetailModal)。
 *
 * 包含:
 * - 头部摘要 DocSummaryCard(可编辑头信息)
 * - 三种产品明细表格 (matrix / no-color-size / variants-only)
 *
 * editing state 由父组件(主壳)通过 helper 注入,setEditing 通过 props 传入。
 */
...
import type { EditingState, useReworkReportFlowDetail } from '../../../hooks/useReworkReportFlowDetail';
...
interface Props {
  ...
  equipmentFeaturesOn: boolean;
  first: ProductionOpRecord;
  order: ProductionOrder | undefined;
  product: Product | undefined;
}

const ReworkEditFlow: React.FC<Props> = ({
  productionLinkMode,
  helper,
  editing,
  setEditing,
  ...
  const totalEditQty = editing.form.rowEdits.reduce((s, r) => s + r.quantity, 0);
  const totalEditAmount = totalEditQty * (editing.form.unitPrice || 0);
          ...
          <>
            <div className="min-w-[6.5rem] md:text-right">
              ...
              <p className="font-black tabular-nums text-slate-800">
                {totalEditQty.toLocaleString()} {unitName}
              </p>
            </div>
            {totalEditAmount > 0 ? (
              <div className="min-w-[6.5rem] md:text-right">
                <p className="text-[10px] text-slate-400 font-black uppercase mb-0.5">金额（元）</p>
                <p className="font-black tabular-nums text-emerald-600">¥{totalEditAmount.toFixed(2)}</p>
              </div>
            ) : null}
          </>
        }
      ...
      <div className="space-y-2">
        ...
        {hasColorSize && reworkFlowMatrixProduct && dictionaries ? (
          <MatrixEditTable
            ...
            first={first}
            unitName={unitName}
            matrixSummaryCustomTags={matrixSummaryCustomTags}
          />
        ) : !hasColorSize ? (
          <NoColorSizeEditTable
            editing={editing}
            setEditing={setEditing}
            product={product}
            first={first}
...
export default ReworkEditFlow;

/* ---------------------- 子表格 ---------------------- */

const MatrixEditTable: React.FC<{
  editing: NonNullable<EditingState>;
  setEditing: React.Dispatch<React.SetStateAction<EditingState>>;
  product: Product | undefined;
  dictionaries: AppDictionaries;
  first: ProductionOpRecord;
  unitName: string;
  matrixSummaryCustomTags: Helper['matrixSummaryCustomTags'];
}> = ({ editing, setEditing, product, dictionaries, first, unitName, matrixSummaryCustomTags }) => {
  const totalQty = editing.form.rowEdits.reduce((s, r) => s + r.quantity, 0);
  const totalAmount = totalQty * (editing.form.unitPrice || 0);
  return (
...
const NoColorSizeEditTable: React.FC<{
  editing: NonNullable<EditingState>;
  setEditing: React.Dispatch<React.SetStateAction<EditingState>>;
  product: Product | undefined;
  first: ProductionOpRecord;
  ...
  matrixSummaryCustomTags: Helper['matrixSummaryCustomTags'];
  productionLinkMode: 'order' | 'product';
}> = ({ editing, setEditing, product, first, order, unitName, matrixSummaryCustomTags, productionLinkMode }) => {
  const totalQty = editing.form.rowEdits.reduce((s, r) => s + r.quantity, 0);
  const totalAmount = totalQty * (editing.form.unitPrice || 0);
  return (
    <div className="overflow-x-auto rounded-2xl border border-slate-200">
                  ...
                  onChange={e => {
                    ...
                    setEditing(prev => {
                      ...
                      if (rows.length === 1) {
                        return {
                          ...prev,
                          form: { ...prev.form, rowEdits: [{ ...rows[0]!, quantity: v }] },
                        };
                      }
...
const VariantsOnlyEditTable: React.FC<{
  editing: NonNullable<EditingState>;
  setEditing: React.Dispatch<React.SetStateAction<EditingState>>;
  unitName: string;
}> = ({ editing, setEditing, unitName }) => (
  <div className="overflow-x-auto rounded-2xl border border-slate-200">
    <table className="w-full text-left text-sm">
      <thead>
        <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 bg-slate-50/80">
