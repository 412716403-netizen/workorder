...
import { ProductionOpRecord, ProductionOrder, Product, GlobalNodeTemplate, AppDictionaries, ProductCategory, ProductVariant, ProductMilestoneProgress } from '../../types';
...
const ReworkDefectiveActionModal: React.FC<ReworkDefectiveActionModalProps> = ({
  ...
  const renderVariantGrid = (mode: 'scrap' | 'rework') => (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="text-[10px] font-bold text-slate-400 uppercase">{mode === 'scrap' ? '报损' : '返工'}数量明细（按规格）</label>
                  ...
                  return (
                    <div key={v.id} className="flex flex-col gap-1 min-w-[64px]">
                      ...
                      <input
                        ...
                        className={`w-full bg-white border border-slate-200 rounded-lg px-2 py-1.5 text-sm font-bold ${mode === 'scrap' ? 'text-rose-600 focus:ring-2 focus:ring-rose-200' : 'text-indigo-600 focus:ring-2 focus:ring-indigo-200'} text-right outline-none placeholder:text-[10px] placeholder:text-slate-400`}
