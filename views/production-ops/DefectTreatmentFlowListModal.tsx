...
const DefectTreatmentFlowListModal: React.FC<DefectTreatmentFlowListModalProps> = ({
  ...
  const [defectFlowFilter, setDefectFlowFilter] = useState<{ dateFrom: string; dateTo: string; orderNumber: string; productId: string; nodeName: string; operator: string; recordType: string }>({ dateFrom: todayDate, dateTo: todayDate, orderNumber: '', productId: '', nodeName: '', operator: '', recordType: '' });

  const flowQuery = useQuery({
    queryKey: ['flow.defect', defectFlowFilter.dateFrom, defectFlowFilter.dateTo],
    queryFn: () =>
      fetchProductionByFilter({
        types: 'REWORK,SCRAP',
        startDate: dateInputToIsoStart(defectFlowFilter.dateFrom),
        endDate: dateInputToIsoEndExclusive(defectFlowFilter.dateTo),
      }),
    staleTime: 15_000,
  });
      ...
      <div className="relative z-10 bg-white w-full max-w-6xl max-h-[min(92vh,960px)] rounded-[32px] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 shrink-0">
          ...
          <div className={`grid grid-cols-2 sm:grid-cols-3 gap-3 ${productionLinkMode === 'product' ? 'md:grid-cols-7' : 'md:grid-cols-8'}`}>
            ...
            <div><label className="text-[10px] font-bold text-slate-400 block mb-1">产品</label><input type="text" value={f.productId} onChange={e => setDefectFlowFilter(prev => ({ ...prev, productId: e.target.value }))} placeholder="产品编号模糊搜索" className="w-full text-sm py-1.5 px-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-200" /></div>
