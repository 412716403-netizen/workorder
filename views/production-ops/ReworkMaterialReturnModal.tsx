...
interface ReworkMaterialReturnModalProps {
...
const ReworkMaterialReturnModal: React.FC<ReworkMaterialReturnModalProps> = ({
  order,
  productName,
  data,
  warehouses,
  ...
  useEffect(() => {
    const pref = readWarehousePreference(
      tenantCtx?.tenantId,
      userId,
      WAREHOUSE_DOC_KIND.PROD_REWORK_MATERIAL_RETURN,
    );
    const fallback = data.returnable[0]?.warehouseId || warehouses[0]?.id || '';
  ...
  }, [tenantCtx?.tenantId, userId, warehouses, data.returnable, currentUser]);
  ...
  const handleSubmit = async () => {
    ...
    for (const row of productRows) {
      ...
      lines.push({
        ...
        quantity: qty,
        warehouseId,
        ...(batchManaged ? { batchNo: bnDisplay } : {}),
      });
    }
    if (lines.length === 0) {
    ...
    setSubmitting(true);
    try {
      const result = await api.production.reworkMaterialReturnBatch(order.id, {
        lines: lines.map(line => ({
          ...line,
          batchNo: batchNoForWrite(line.batchNo) ?? line.batchNo,
        })),
        operator: operatorName,
        timestamp: entryDatetimeLocalToTimestamp(entryTimestamp),
      });
      writeWarehousePreference(tenantCtx?.tenantId, userId, WAREHOUSE_DOC_KIND.PROD_REWORK_MATERIAL_RETURN, {
        warehouseId,
      });
      toast.success(`退料成功 · ${result.docNo}`);
      await onSaved();
    } catch (err) {
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
            <div>
              <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <ArrowDownToLine className="w-5 h-5 text-indigo-600" /> 返工退料
              </h3>
...
export default ReworkMaterialReturnModal;
