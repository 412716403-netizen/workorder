...
export interface PurchaseOrderDetailSummaryProps {
  docNumber: string;
  recordsList: any[];
  ...
  /** 与列表行 `${docNum}::${lineId}` 一致 */
  receivedByOrderLine: Record<string, number>;
}

const PurchaseOrderDetailSummary: React.FC<PurchaseOrderDetailSummaryProps> = ({
  docNumber,
  recordsList,
  productMapPSI,
  dictionaries,
  ...
  if (!mainInfo) {
    return <p className="text-sm text-slate-500 py-8 text-center">未找到该采购订单数据。</p>;
  }
        ...
        <table className="w-full text-left text-sm" style={{ tableLayout: 'fixed' }}>
          ...
          <thead>
            <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 bg-slate-50/80">
              ...
              <th className="py-2.5 px-3 text-left">入库进度</th>
            </tr>
          </thead>
...
export default React.memo(PurchaseOrderDetailSummary);
