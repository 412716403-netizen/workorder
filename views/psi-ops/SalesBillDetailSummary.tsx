...
export interface SalesBillDetailSummaryProps {
  docNumber: string;
  recordsList: any[];
...
const SalesBillDetailSummary: React.FC<SalesBillDetailSummaryProps> = ({
  docNumber,
  recordsList,
  productMapPSI,
  warehouseMapPSI,
  ...
  if (!mainInfo) {
    return <p className="text-sm text-slate-500 py-8 text-center">未找到该销售单数据。</p>;
  }
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 sm:p-5 space-y-3">
        ...
        <div className="flex flex-wrap gap-6 pt-1 border-t border-slate-200/80 text-sm">
          <div>
            ...
            <p className={`font-black tabular-nums ${totalQty < 0 ? 'text-amber-600' : 'text-slate-800'}`}>
              {totalQty.toLocaleString()} PCS
            </p>
          </div>
          <div>
            <p className="text-[10px] text-slate-400 font-black uppercase mb-0.5">合计金额</p>
            <p className={`font-black tabular-nums ${totalAmount < 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
        ...
        <table className="w-full text-left text-sm" style={{ tableLayout: 'fixed' }}>
          ...
          <thead>
            <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 bg-slate-50/80">
              ...
              <th className="py-2.5 px-3 text-center">出库仓库</th>
...
export default React.memo(SalesBillDetailSummary);
