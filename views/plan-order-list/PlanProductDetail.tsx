                  ...
                  if (f.type === 'file' && typeof val === 'string' && val.startsWith('data:')) {
                    ...
                    if (isImg) return (
                      <div key={f.id} className="flex items-center gap-2">
                        <img src={val} alt={f.label} className="h-12 w-12 object-cover rounded-xl border cursor-pointer hover:ring-2 hover:ring-indigo-400" onClick={() => onFilePreview(val, 'image')} />
                        <a href={val} download={`${f.label}.${getFileExtFromDataUrl(val)}`} className="text-xs font-bold text-indigo-600 hover:underline">下载</a>
                      </div>
                    );
                    if (isPdf) return (
                      <div key={f.id} className="flex items-center gap-2">
                        <button type="button" onClick={() => onFilePreview(val, 'pdf')} className="px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-bold hover:bg-indigo-100">在线查看</button>
                        <a href={val} download={`${f.label}.${getFileExtFromDataUrl(val)}`} className="text-xs font-bold text-indigo-600 hover:underline">下载</a>
                      </div>
                    );
                    return (
