...
import { requireSubPermission, requirePsiOrProductionRead, requirePsiOrProductionOrDevelopmentRead, requirePsiRecordWrite } from '../middleware/tenant.js';
...
// 只读库存聚合：生产计划/工单/物料/进销存/开发领退料多处面板共用。
// 详见 middleware/tenant.ts requirePsiOrProductionOrDevelopmentRead 说明。
router.get('/stock', requirePsiOrProductionOrDevelopmentRead(), ctrl.getStock);
router.get('/stock/batches', requirePsiOrProductionOrDevelopmentRead(), ctrl.getStockBatches);
router.get('/stock-snapshot', requirePsiOrProductionOrDevelopmentRead(), ctrl.getStockSnapshot);
