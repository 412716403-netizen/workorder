...
export const listPartners = asyncHandler(async (req, res) => {
  ...
  if (all) warnListAllFromRequest('masterData.listPartners', req);
  res.json(await masterDataService.listPartners(db, { categoryId: optStr(req.query.categoryId), search: optStr(req.query.search), all, page, pageSize }));
});
...
export const listWorkers = asyncHandler(async (req, res) => {
  const db = getTenantPrisma(req.tenantId!);
  const { all, page, pageSize } = listQueryFromRequest(req);
  if (all) warnListAllFromRequest('masterData.listWorkers', req);
  res.json(await masterDataService.listWorkers(db, { status: optStr(req.query.status), search: optStr(req.query.search), all, page, pageSize }));
});
...
export const listEquipment = asyncHandler(async (req, res) => {
  const db = getTenantPrisma(req.tenantId!);
  const { all, page, pageSize } = listQueryFromRequest(req);
  if (all) warnListAllFromRequest('masterData.listEquipment', req);
  res.json(await masterDataService.listEquipment(db, { search: optStr(req.query.search), all, page, pageSize }));
});
...
export const listDictionaries = asyncHandler(async (req, res) => {
  ...
  res.json(await masterDataService.listDictionaries(getTenantPrisma(tenantId), tenantId));
});
export const createDictionaryItem = asyncHandler(async (req, res) => {
  res.status(201).json(await masterDataService.createDictionaryItem(getTenantPrisma(req.tenantId!), req.tenantId!, req.body));
});
export const updateDictionaryItem = asyncHandler(async (req, res) => {
  const result = await masterDataService.updateDictionaryItem(getTenantPrisma(req.tenantId!), req.tenantId!, str(req.params.id), req.body);
...
export const deleteDictionaryItem = asyncHandler(async (req, res) => {
  res.json(await masterDataService.deleteDictionaryItem(getTenantPrisma(req.tenantId!), req.tenantId!, str(req.params.id)));
});
