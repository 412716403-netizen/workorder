...
export const listTodos = asyncHandler(async (req, res) => {
  ...
  res.json(await svc.listTodos(db, tenantId, userId, { status }));
});
...
export const updateTodo = asyncHandler(async (req, res) => {
  ...
  const db = getTenantPrisma(tenantId);
  const userId = req.user!.userId;
  res.json(await svc.updateTodo(db, tenantId, userId, str(req.params.id), req.body));
});
...
export const deleteTodo = asyncHandler(async (req, res) => {
  ...
  const db = getTenantPrisma(tenantId);
  const userId = req.user!.userId;
  res.json(await svc.deleteTodo(db, tenantId, userId, str(req.params.id)));
});
