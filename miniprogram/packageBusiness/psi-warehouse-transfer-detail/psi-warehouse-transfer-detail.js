...
const _require3 = require('../config/warehouses.js'),PSI_TRANSFER_TYPE = _require3.PSI_TRANSFER_TYPE;
const _require4 = require('../utils/warehouseTransfer.js'),mapTransferDetailView = _require4.mapTransferDetailView;
...
Page({
  ...
  onLoad(options) {
    ...
    const canEdit = hasPermission(ctx && ctx.permissions || [], 'psi:warehouse_transfer:edit');
    const canDelete = hasPermission(ctx && ctx.permissions || [], 'psi:warehouse_transfer:delete');
  ...
  onEditTap() {
    ...
    wx.navigateTo({
      url: `/packageBusiness/psi-warehouse-transfer-edit/psi-warehouse-transfer-edit?docNumber=${encodeURIComponent(this.data.docNumber)}`
    });
  },
  ...
  async loadDetail() {
    ...
    try {
      ...
      const view = mapTransferDetailView(
        this.data.docNumber,
        items,
      ...
      this.setData({
        ...
        title: view.hero.docNumber || '调拨单详情'
      });
    } catch (err) {
