...
const _require3 =
  ...
  require('../utils/financeReceipts.js'),emptyReceiptForm = _require3.emptyReceiptForm,formFromRecord = _require3.formFromRecord,formVisibility = _require3.formVisibility,validateReceiptForm = _require3.validateReceiptForm,buildReceiptSavePayload = _require3.buildReceiptSavePayload,buildCategoryPickerOptions = _require3.buildCategoryPickerOptions,buildAccountPickerOptions = _require3.buildAccountPickerOptions,buildWorkerPickerOptions = _require3.buildWorkerPickerOptions,initialCustomDataForCategory = _require3.initialCustomDataForCategory,categoriesForReceipt = _require3.categoriesForReceipt;
const _require4 =
...
Page({
  ...
  onLoad(options) {
    ...
    const perm = editing ? 'finance:receipt:edit' : 'finance:receipt:create';
  ...
  async bootstrap() {
    ...
    try {
      ...
      const categories = categoriesForReceipt(normalizeMasterList(categoriesRaw));
      ...
      this._existing = existing && existing.type === 'RECEIPT' ? existing : null;
  ...
  applyVisibility(form) {
    ...
    const canSubmit = !validateReceiptForm(form, visibility);
  ...
  onSubmit() {
    ...
    const err = validateReceiptForm(form, visibility);
    if (err) {
