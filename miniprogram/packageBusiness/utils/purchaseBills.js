/**
 * 采购入库 view-model（对齐 Web PSIOpsView / PsiDocDetailSummary PURCHASE_BILL）
 */
...
const { formatPsiDocNumForList, formatPsiDocBusinessDateListZh } = require('./purchaseOrders.js');

function parsePurchaseBillSearch(raw) {
...
function docMatchesSearch(docNumber, items, keyword, productMap, warehouseMap) {
  ...
  (items || []).forEach((r) => {
    if (r.sourceOrderNumber) fields.push(String(r.sourceOrderNumber));
...
function sortPurchaseBillDocEntries(groups) {
...
function filterPurchaseBillDocs(groups, keyword, productMap, warehouseMap) {
  let entries = Object.entries(groups);
  if (keyword) {
    entries = entries.filter(([docNumber, items]) =>
      docMatchesSearch(docNumber, items, keyword, productMap, warehouseMap));
  }
  return sortPurchaseBillDocEntries(Object.fromEntries(entries));
}

function readSourceOrderNumber(items) {
  for (const r of items || []) {
    if (r.sourceOrderNumber) return String(r.sourceOrderNumber).trim();
  }
  return '';
}
...
function mapLineGroupPreview(lineGroupId, items, ctx) {
  ...
  return {
    ...
    sourceOrderNumber: first.sourceOrderNumber ? String(first.sourceOrderNumber) : '',
    showSourceOrder: Boolean(first.sourceOrderNumber),
  };
}

function mapPurchaseBillListCard(docNumber, items, ctx) {
  ...
  const sourceOrder = readSourceOrderNumber(items);
  ...
  return {
    ...
    sourceOrder,
    showSourceOrder: Boolean(sourceOrder),
...
function buildPurchaseBillListCards(groups, keyword, ctx) {
  const entries = filterPurchaseBillDocs(
    groups,
    keyword,
    ctx.productMap,
    ctx.warehouseMap,
  ...
  return entries.map(([docNumber, docItems]) =>
    mapPurchaseBillListCard(docNumber, docItems, ctx));
}

function slimPurchaseBillLinePreview(line) {
  return {
    ...
    showSourceOrder: line.showSourceOrder,
  };
}

function slimPurchaseBillListCard(card) {
  return {
    ...
    showSourceOrder: card.showSourceOrder,
    ...
    lines: (card.lines || []).map(slimPurchaseBillLinePreview),
  };
}
...
function buildDetailLineRows(items, ctx) {
  ...
  Object.entries(lineGroups).forEach(([lineGroupId, grp]) => {
    ...
    rows.push({
      ...
      sourceOrderNumber: first.sourceOrderNumber ? String(first.sourceOrderNumber) : '',
      showSourceOrder: Boolean(first.sourceOrderNumber),
      lineNote: first.note ? String(first.note) : '',
      showLineNote: Boolean(first.note),
    });
  });
...
function mapPurchaseBillDetailView(docNumber, items, ctx) {
  const { showAmount, warehouseMap } = ctx;
  const first = items[0] || {};
  const totalQty = docTotalQty(items);
  const isReturn = totalQty < 0;
  ...
  const hero = {
    ...
    docNumberDisplay: formatPsiDocNumForList(docNumber),
    showStatusPill: isReturn,
    statusLabel: isReturn ? '采购退货' : '采购入库',
...
function buildPurchaseBillFlowRows(records, ctx) {
  ...
  Object.entries(grouped).forEach(([docNumber, docItems]) => {
    ...
    Object.entries(lineGroups).forEach(([lineGroupId, items]) => {
      rows.push(mapPurchaseBillFlowRow(docNumber, lineGroupId, items, ctx));
    });
  });
...
function mapPurchaseBillFlowRow(docNumber, lineGroupId, items, ctx) {
...
function filterPurchaseBillFlowRows(rows, filters) {
  let list = rows || [];
...
function computePurchaseBillFlowStats(rows) {
