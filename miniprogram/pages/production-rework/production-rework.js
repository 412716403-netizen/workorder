...
Page({
  ...
  onLoad(options) {
    ...
    if (reworkOrderId) {
      wx.redirectTo({
        url: `/pages/production-rework-detail/production-rework-detail?reworkOrderId=${encodeURIComponent(reworkOrderId)}`,
      });
      return;
    }
  ...
  onChipTap(e) {
    ...
    wx.navigateTo({
      url: `/pages/production-rework-report/production-rework-report?${q.join('&')}`,
    });
  },
  ...
  onMaterialTap(e) {
    ...
    if (this._productionLinkMode === 'product' && productId) {
      wx.navigateTo({
        url: `/pages/production-order-material/production-order-material?source=rework&productId=${encodeURIComponent(productId)}`,
      });
      return;
    }
    if (!orderId) return;
    wx.navigateTo({
      url: `/pages/production-order-material/production-order-material?source=rework&orderId=${encodeURIComponent(orderId)}`,
    });
  },
