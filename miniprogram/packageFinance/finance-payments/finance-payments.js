...
Page({
  ...
  onLoad(options) {
    ...
    if (id) {
      wx.redirectTo({
        url: `/packageFinance/finance-payment-detail/finance-payment-detail?id=${encodeURIComponent(id)}`
      });
      return;
    }
  ...
  onShow() {
    ...
    this.setData({
      canViewFlow: hasDocViewPermission(ctx.permissions || [], 'finance:payment:view'),
  ...
  onCreateTap() {
    ...
    wx.navigateTo({ url: '/packageFinance/finance-payment-edit/finance-payment-edit' });
  },

  onFlowTap() {
    wx.navigateTo({ url: '/packageFinance/finance-payment-flow/finance-payment-flow' });
  },
