...
Page({
  ...
  onLoad(options) {
    ...
    if (id) {
      wx.redirectTo({
        url: `/packageFinance/finance-receipt-detail/finance-receipt-detail?id=${encodeURIComponent(id)}`
      });
      return;
    }
  ...
  onShow() {
    ...
    if (!hasDocViewPermission(ctx.permissions || [], 'finance:receipt:view')) {
      wx.showToast({ title: '无权限', icon: 'none' });
      setTimeout(() => wx.navigateBack(), 800);
      return;
    }
    this.setData({
      canViewFlow: hasDocViewPermission(ctx.permissions || [], 'finance:receipt:view'),
  ...
  onCreateTap() {
    ...
    wx.navigateTo({ url: '/packageFinance/finance-receipt-edit/finance-receipt-edit' });
  },

  onFlowTap() {
    wx.navigateTo({ url: '/packageFinance/finance-receipt-flow/finance-receipt-flow' });
  },
