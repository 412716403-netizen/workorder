...
function buildFlowDetailUrl(row) {
  ...
  switch (row.type) {
    case 'PURCHASE_BILL':
      return `/pages/psi-purchase-bill-detail/psi-purchase-bill-detail?docNumber=${doc}`;
    case 'SALES_BILL':
      return `/pages/psi-sales-bill-detail/psi-sales-bill-detail?docNumber=${doc}`;
    case 'TRANSFER':
      return `/pages/psi-warehouse-transfer-detail/psi-warehouse-transfer-detail?docNumber=${doc}`;
    case 'STOCKTAKE':
      return `/pages/psi-warehouse-stocktake-detail/psi-warehouse-stocktake-detail?docNumber=${doc}`;
    case 'STOCK_IN': {
      const rec = row.record || {};
      ...
      return `/pages/production-order-stock-in-detail/production-order-stock-in-detail?${q.join('&')}`;
    }
    case 'STOCK_OUT':
    case 'STOCK_RETURN': {
      const rec = row.record || {};
      const docNo = rec.docNo || row.docNumber || '';
      return `/pages/production-stock-out-detail/production-stock-out-detail?docNo=${encodeURIComponent(docNo)}`;
    }
    default:
      return '';
  }
