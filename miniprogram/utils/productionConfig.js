const _require =
  ...
  require('../config/productionConfig.js'),PRODUCT_MATERIAL_COST_MODES = _require.PRODUCT_MATERIAL_COST_MODES,MATERIAL_COST_MODE_LABEL = _require.MATERIAL_COST_MODE_LABEL,MATERIAL_COST_MODE_DESC = _require.MATERIAL_COST_MODE_DESC,DEFAULT_WEIGHT_TOLERANCE_PERCENT = _require.DEFAULT_WEIGHT_TOLERANCE_PERCENT;
 ...
 */
function buildProductionConfigForm(config, plugins) {var _cfg$weightToleranceP;
  const cfg = config || {};
  ...
  return {
    exceedToggles,
    weightEnabled,
    weightTolerancePercent: clampWeightTolerance((_cfg$weightToleranceP =
    cfg.weightTolerancePercent) != null ? _cfg$weightToleranceP : DEFAULT_WEIGHT_TOLERANCE_PERCENT
    ),
    costModes,
