...
function computeWarehouseFlowRows(input) {
  const
    ...
    input.recordsList,prodRecords = input.prodRecords,productMap = input.productMap,warehouseMap = input.warehouseMap,ordersList = input.ordersList;

  const list = (recordsList || []).filter((r) => WAREHOUSE_FLOW_TYPES.includes(r.type));
  const psiRows = list.map((r) => {var _r$quantity;
    ...
    const qty = (_r$quantity = r.quantity) != null ? _r$quantity : 0;
  ...
  function buildProdRow(type, typeLabel, fallbackDocPrefix, isOutbound) {
    return (prodRecords || []).filter((r) => r.type === type).map((r) => {var _r$quantity2;
      ...
      return {
        ...
        productName: product && product.name || '—',
        productSku: product && product.sku || '—',
        quantity: (_r$quantity2 = r.quantity) != null ? _r$quantity2 : 0,
...
function computeWarehouseFlowTotals(rows) {
  ...
  (rows || []).forEach((row) => {
    ...
    switch (row.type) {
      case 'PURCHASE_BILL':
        if (qty >= 0) inboundTotal += qty;else
      ...
      case 'STOCK_OUT':
        ...
        break;
      case 'SALES_BILL':
        if (qty >= 0) outboundTotal += qty;else
        inboundTotal += Math.abs(qty);
        break;
      case 'STOCKTAKE':{var _ref, _rec$diffQuantity;
          const rec = row.record || {};
          const diff = Number((_ref = (_rec$diffQuantity = rec.diffQuantity) != null ? _rec$diffQuantity : rec.diff_quantity) != null ? _ref : 0);
          if (diff > 0) inboundTotal += diff;else
          if (diff < 0) outboundTotal += Math.abs(diff);
          break;
        }
      default:
        break;
...
function filterWarehouseFlowRows(rows, filters) {
  const _ref2 =
    ...
    filters || {},searchKeyword = _ref2.searchKeyword,dateFrom = _ref2.dateFrom,dateTo = _ref2.dateTo,typeFilter = _ref2.typeFilter,warehouseId = _ref2.warehouseId,docNumber = _ref2.docNumber;
...
function buildFlowDetailUrl(row) {
  ...
  switch (row.type) {
    case 'PURCHASE_BILL':
      return `/packagePsi/psi-purchase-bill-detail/psi-purchase-bill-detail?docNumber=${doc}`;
    case 'SALES_BILL':
      return `/packagePsi/psi-sales-bill-detail/psi-sales-bill-detail?docNumber=${doc}`;
    case 'TRANSFER':
      return `/packagePsi/psi-warehouse-transfer-detail/psi-warehouse-transfer-detail?docNumber=${doc}`;
    case 'STOCKTAKE':
      return `/packagePsi/psi-warehouse-stocktake-detail/psi-warehouse-stocktake-detail?docNumber=${doc}`;
    case 'STOCK_IN':{
        const rec = row.record || {};
