...
function buildStockSnapshotIndex(data) {
  ...
  function getStock(pId, whId, excludeStocktakeDocNumber) {
    if (!whId) return 0;
    const b = whMap.get(`${pId}::${whId}`);
    if (!b) return 0;
    let adj = b.stocktakeAdj || 0;
    if (excludeStocktakeDocNumber && adj !== 0) {
      // snapshot 已聚合；编辑盘点时 exclude 由后端 snapshot 口径覆盖，客户端沿用 snapshot
    }
    return netFromBucket(b) + adj;
  }
