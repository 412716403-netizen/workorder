...
const _require2 = require('../../utils/permissions.js'),hasPermission = _require2.hasPermission,hasDocViewPermission = _require2.hasDocViewPermission,filterByPermission = _require2.filterByPermission;
const _require3 =
...
Page({
  ...
  onShow() {
    ...
    if (!hasDocViewPermission(ctx.permissions || [], 'psi:sales_order:view')) {
  ...
  onCreateTap() {
    ...
    wx.navigateTo({ url: '/packagePsi/psi-sales-order-edit/psi-sales-order-edit' });
  },
  ...
  onShortcutTap(e) {
    ...
    if (id === 'order-flow') {
      wx.navigateTo({ url: '/packagePsi/psi-sales-order-flow/psi-sales-order-flow' });
      return;
    }
    if (id === 'pending-ship') {
      wx.navigateTo({ url: '/packagePsi/psi-sales-order-pending-ship/psi-sales-order-pending-ship' });
    }
  },
  ...
  onAllocateTap(e) {
    ...
    wx.navigateTo({
      url: `/packagePsi/psi-sales-order-allocate/psi-sales-order-allocate?docNumber=${encodeURIComponent(docNumber)}&lineGroupId=${encodeURIComponent(lineGroupId)}`
    });
  },
