...
const { canViewSettingsTab } = require('../utils/accessControl.js');
...
function canViewTab(permissions, tenantRole, tab) {
  return canViewSettingsTab(permissions, tenantRole, tab.permission);
}
 ...
 */
function buildProductionConfigSections(config) {var _config$weightToleran;
  ...
  },
  {
    ...
    {
      label: '扫码称重容差',
      value: `${(_config$weightToleran = config.weightTolerancePercent) != null ? _config$weightToleran : 5}%`
    }]
